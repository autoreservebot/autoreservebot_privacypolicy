...
INTERNET: El acceso a la red se limita exclusivamente a la telemetría técnica anónima y al uso de servicios de diagnóstico técnico: Firebase Crashlytics y Remote Config, estos servicios solo recopilan identificadores técnicos de dispositivos y registros de fallos. Estos datos están completamente anonimizados y no pueden vincularse a la identidad del usuario ni al contenido de los mensajes procesados ​​por el bot. Aunque la aplicación tiene permiso para conectarse a Internet, todas sus funciones operan exclusivamente de forma local; ningún dato dentro de la aplicación se transmite a través de Internet. Por lo tanto, los datos personales y cualquier otro dato nunca se envían fuera de la aplicación.

READ_CONTACT: Este permiso se utiliza en modo de solo lectura para permitir al usuario ver sus contactos dentro de la aplicación y seleccionar sus contactos para autorizarlos a interactuar con el bot y programar citas. Esto solo ocurre si el usuario elige "WhatsApp" como plataforma de respuesta automática. No se crea ninguna copia de la libreta de direcciones fuera de la base de datos local cifrada por el sistema Android. No almacenamos datos de contacto en servidores externos ni los compartimos de ninguna manera.
...
INTERNET: L'accesso alla rete è limitato esclusivamente alla telemetria tecnica anonima e per l'utilizzo dei servizi di diagnostica tecnica : Firebase Crashlytics, Remote Config, infatti quest' ultimi raccolgono solo identificativi tecnici del dispositivo e log di crash. Questi dati sono totalmente de-identificati e non possono essere collegati all'identità dell'utente o ai contenuti dei messaggi elaborati dal bot. Sebbene l'applicazione abbia l'autorizzazione a connettersi a Internet, tutte le funzionalità dell'app operano esclusivamente in locale, infatti tutti i dati che transitano all'interno dell'applicazione non vengono trasmessi in Internet. Dunque, i dati personali e tutti gli altri dati non vengono mai inviati all'esterno.

READ_CONTACT:
Questo permesso viene utilizzato in modalità sola lettura per permettere all'utente di visualizzare la propria rubrica all'interno dell'app, e selezionare i suoi contatti per autorizzarli a interagire con il bot per fissare appuntamenti, ciò avviene esclusivamente se l'utente sceglie "WhatsApp" come piattaforma di risposta automatica. Nessuna copia della rubrica viene creata al di fuori del database locale crittografato dal sistema Android. I dati dei contatti non vengono da noi memorizzati su server esterni né condivisi in alcun modo.
